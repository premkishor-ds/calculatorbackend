...
function buildScreenerQuery(query, asOfDate) {
  ...
  // Sprint 5: Smart Watchlists Indicators Logic
  const smart = query.smartType || '';
  if (smart === 'high_volume') {
    mongo.volume = { $gte: 2000000 };
  } else if (smart === 'breakout') {
    mongo.changePercent = { $gte: 4.5 };
  } else if (smart === 'oversold') {
    mongo.pe = { $lte: 12 };
  } else if (smart === 'overbought') {
    mongo.pe = { $gte: 45 };
  } else if (smart === 'momentum') {
    mongo.changePercent = { $gte: 2.0 };
    mongo.volume = { $gte: 500000 };
  }
